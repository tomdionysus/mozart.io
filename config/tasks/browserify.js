...
module.exports = function (grunt) {
  ...
  grunt.registerMultiTask('browserify', 'Your task description goes here.', function () {
    ...
    grunt.file.expand({filter: "isFile"}, this.data.entries || []).forEach(function (filepath) {
      grunt.verbose.writeln('Adding "' + filepath + '" to the entry file list');
      b.addEntry(filepath);
    });

    var files = grunt.file.expand({filter:"isFile"}, this.data.prepend || []);
    var src = legacyHelpers.concat(files, {
      separator: ''
    });
    b.prepend(src);

    files = grunt.file.expand({filter:"isFile"},this.data.append || []);
    src = legacyHelpers.concat(files, {
      separator: ''
    });
    b.append(src);
