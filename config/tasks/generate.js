...
/**
 * Custom task for generating Mozart components.
 */
module.exports = function(grunt){

    var VALID_TYPES = ['controller', 'model', 'view', 'control', 'templates/view', 'templates/control'];
    ...
    grunt.registerTask('generate', 'Generates various Mozart objects based on templates.', function(type, name, path){
        ...
        if(type == 'control'){ // Controllers are nested in views
          folder = grunt.config('generate.root') + 'views/' + type + 's/';
        } 
        else if(type == 'templates/view'){ // Views are in the templates directory
          folder = grunt.config('generate.root') + 'templates/';
        } 
        else { // Everything else
          folder = grunt.config('generate.root') + type + 's/';
        }
        ...
        try {
            ...
            if (info.isDirectory()){
                ...
                var suffix = '';
                switch (type){
                    case 'model': suffix = '.coffee'; break;
                    case 'templates/view': suffix = '_view.hbs'; break;
                    case 'templates/control': suffix = '_control.hbs'; break;
                    default: suffix = '_' + type + '.coffee';
                }
                filename += suffix;
                ...
                if (!fs.existsSync(filename)) {
                    ...
                    // If we just created a view or control, we also need to create a template.
                    // TODO: Externalise this dependency logic.
                    if (type == 'view') {
                        grunt.task.run('generate:templates/view:' + name + ":" + path);
                    }
                    else if (type == 'control') {
                        grunt.task.run('generate:templates/control:' + name + ":" + path);
                    }
